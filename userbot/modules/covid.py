...
async def corona(event):
    ...
    if country_data:
        output_text = f"`⚠️Confirmed   : {
            format_integer(
                country_data['confirmed'])}`\n"
        output_text += f"`☢️Active      : {
            format_integer(
                country_data['active'])}`\n"
        output_text += f"`🤕Critical    : {
            format_integer(
                country_data['critical'])}`\n"
        output_text += f"`😟New Deaths  : {
                format_integer(
                    country_data['new_deaths'])}`\n\n"
        output_text += f"`⚰️Deaths      : {
            format_integer(
                country_data['deaths'])}`\n"
        output_text += f"`😔New Cases   : {
            format_integer(
                country_data['new_cases'])}`\n"
        output_text += f"`😇Recovered   : {
            format_integer(
                country_data['recovered'])}`\n"
        output_text += f"`🧪Total tests : {
                format_integer(
                    country_data['total_tests'])}`\n\n"
